...
func init() {
	...
	edinEndpoint = data["edinEndpoint"]
	acessKeyId = data["acessKeyId"]
	accessKeySecret = data["accessKeySecret"]
	bucketName = data["bucketName"]

	client, err := oss.New(edinEndpoint, acessKeyId, accessKeySecret)
	handleError(err)
...
func Upload(path string, fileName string, file io.Reader) string {
	...
	return "https://" + bucketName + "." + edinEndpoint + "/" + objectName
}
