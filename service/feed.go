...
func GetFeedVideoList(currentUserId int64, lastTime time.Time) VideoList {
	...
	err := Db.Select(&workList, "SELECT id, user_id, play_url, cover_url,title, create_time FROM user_work WHERE user_id != ? AND deleted = 0 AND create_time < ?  LIMIT 0, ?", currentUserId, util.TimeFormat(lastTime), constant.FEED_VIDEO_LIST_SIZE)
	HandleSqlError(err)
